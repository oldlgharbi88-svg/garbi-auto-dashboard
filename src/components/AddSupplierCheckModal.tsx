...
import type { NewSupplierCheck } from '../lib/types';

interface AddSupplierCheckModalProps {
  open: boolean;
  onClose: () => void;
  onSave: (check: NewSupplierCheck) => Promise<void>;
}
...
export default function AddSupplierCheckModal({ open, onClose, onSave }: AddSupplierCheckModalProps) {
  ...
  const handleSubmit = async () => {
    if (!supplierName.trim() || Number(amount) <= 0 || !dueDate) {
      setError('Please complete the supplier name, amount, and due date.');
      return;
    }
    ...
    try {
      ...
      console.error(err);
      setError('Unable to save the supplier check. Please try again.');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4 py-8">
      <div className="w-full max-w-2xl overflow-hidden rounded-3xl border border-outline-variant bg-surface-container p-6 shadow-2xl shadow-black/30">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-2xl font-semibold text-on-surface">Nouveau chèque fournisseur</h2>
            <p className="mt-2 text-sm text-on-surface-variant">Saisissez les informations du chèque et enregistrez-le.</p>
          </div>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          <label className="space-y-2 text-sm text-on-surface-variant">
            Supplier
            <input className={inputClasses} value={supplierName} onChange={(event) => setSupplierName(event.target.value)} placeholder="Supplier name" />
          </label>
          <label className="space-y-2 text-sm text-on-surface-variant">
            Reference
          ...
          <label className="space-y-2 text-sm text-on-surface-variant">
            Phone
            <input className={inputClasses} value={supplierPhone} onChange={(event) => setSupplierPhone(event.target.value)} placeholder="Supplier phone" />
          </label>
          <label className="space-y-2 text-sm text-on-surface-variant">
            Email
            <input className={inputClasses} type="email" value={supplierEmail} onChange={(event) => setSupplierEmail(event.target.value)} placeholder="Supplier email" />
          </label>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <button type="button" disabled={saving} onClick={handleSubmit} className="rounded-2xl bg-primary px-5 py-3 text-sm font-semibold text-on-primary transition hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-60">
            {saving ? 'Saving…' : 'Save supplier check'}
          </button>
